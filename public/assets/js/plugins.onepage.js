...
SEMICOLON.widget.onePageModule = function( $onePageMenuEl ){
	...
	windowEl.scrollEnd( function(){
		SEMICOLON.widget.onePageScroller();
	}, 500 );
	...
	$onePageMenu.each( function(){
		...
		linkEls.each( function(){
			...
			let linkEl			= $(this),
				...
				linkElSettings	= SEMICOLON.widget.onePageSettings( linkElAnchor, $('a[data-href="'+ linkElAnchor +'"]') );
		...
		linkEls.off( 'click' ).on( 'click', function(){
			...
			let linkEl			= $(this),
				linkElAnchor	= linkEl.attr('data-href'),
				linkElement		= $( linkElAnchor ),
				linkElementOff	= linkElement.offset().top;
			...
			if( linkElement.length > 0 ) {
				...
				if( windowWidth < 768 || $body.hasClass('overlay-menu') ) {
					if( $('.primary-menu').find('.menu-container.mobile-primary-menu').length > 0 ) {
						$('.menu-container.mobile-primary-menu').toggleClass('d-block', false);
					} else {
						$('.menu-container').toggleClass('d-block', false);
					}
...
SEMICOLON.widget.onePageScroller = function(){
	$onePageMenu.find('[data-href]').parent().removeClass( onePageMenuActiveClass );
	$onePageMenu.find('[data-href="#' + SEMICOLON.widget.onePageCurrentSection() + '"]').parent().addClass( onePageMenuActiveClass );
};

SEMICOLON.widget.onePageCurrentSection = function(){
...
SEMICOLON.widget.onePageSettings = function( hash, element ) {
