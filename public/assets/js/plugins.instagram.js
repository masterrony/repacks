...
SEMICOLON.widget.instagramPhotosInit = function( $instagramPhotosEl ){
	...
	$instagramPhotosEl.each(function() {
		...
		SEMICOLON.widget.getInstagramPhotos( element, elUsername, elLimit );
	});

};


SEMICOLON.widget.getInstagramPhotos = function( element, username, limit, nextkey = '', images = false ) {
	...
	$.getJSON( 'https://www.instagram.com/'+ username +'/?__a=1'+nextkey, function( instagram ){
		...
		if( !instaPrivate ) {
			...
			if( images.length < limit ) {
				images = SEMICOLON.widget.getInstagramPhotos( element, username, limit, nextkey, images );
			}
